...
export function UrbanTheme({ data }: { data: PublicSiteData }) {
  if (data.content) return <EditableSite data={data} defaults={urbanTokens} variant="urban" />;
  return <ThemeShell data={data} defaults={urbanTokens} variant="urban"><div id="inicio" /><SiteNavigation data={data} /><main>
    ...
    <ThemeSection id="agendamento"><Container><SectionHeading label="Agendamento" title="MARQUE SEU HORÁRIO." /><BookingWidget locationId={data.tenant.location.id} timezone={data.tenant.location.timezone} services={data.services || []} professionals={data.professionals || []} preview={data.preview} /></Container></ThemeSection>
