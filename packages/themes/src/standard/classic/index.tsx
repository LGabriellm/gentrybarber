...
export function ClassicTheme({ data }: { data: PublicSiteData }) {
  if (data.content) return <EditableSite data={data} defaults={classicTokens} variant="classic" />;
  return <ThemeShell data={data} defaults={classicTokens} variant="classic"><div id="inicio" /><SiteNavigation data={data} />
    ...
    <ThemeSection id="agendamento"><Container><SectionHeading label="Marque seu horário" title="Agende agora." /><BookingWidget locationId={data.tenant.location.id} timezone={data.tenant.location.timezone} services={data.services || []} professionals={data.professionals || []} preview={data.preview} /></Container></ThemeSection>
