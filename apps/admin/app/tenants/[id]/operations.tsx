...
export function TenantOperations({ tenantId, services, professionals, initialSchedule }: { tenantId: string; services: ServiceCatalog; professionals: ProfessionalCatalog; initialSchedule: Schedule | null }) {
  ...
  const [hours, setHours] = useState<DraftWindow[]>(draftWindows(initialSchedule?.businessHours ?? []));
  ...
  async function loadSchedule(locationId: string) {
    ...
    if (result.data) { setSchedule(result.data); setHours(draftWindows(result.data.businessHours)); }
  ...
  async function saveSchedule(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true);
    const result = await adminTenantOperation<Schedule>(tenantId, 'schedule', 'PUT', { locationId: schedule.location.id, expectedVersion: schedule.location.version, businessHours: windows, professionals: [] });
    setMessage({ text: result.error ?? 'Expediente semanal salvo.', error: !!result.error }); if (result.data) { setSchedule(result.data); setHours(draftWindows(result.data.businessHours)); } setBusy(false);
  }
  return <>
    ...
    <section id="horarios" className="admin-panel"><div className="admin-panel-heading"><div><h2>Expediente semanal</h2><p>Defina todos os intervalos da unidade. Use dois intervalos no mesmo dia para representar uma pausa.</p></div>{services.locations.length > 1 && <label>Unidade<select value={schedule?.location.id ?? ''} disabled={busy} onChange={event => loadSchedule(event.target.value)}>{services.locations.map(location => <option key={location.id} value={location.id}>{location.name}</option>)}</select></label>}</div>{schedule ? <form className="admin-form" noValidate onSubmit={saveSchedule}><fieldset disabled={busy}><div className="admin-hours-grid">{hours.map((window, index) => <div className="admin-hours-row" key={`${schedule.location.id}-${index}`}><label>Dia<select value={window.weekday} onChange={event => setHours(previous => previous.map((item, position) => position === index ? { ...item, weekday: Number(event.target.value) } : item))}>{weekdays.map((day, weekday) => <option key={day} value={weekday}>{day}</option>)}</select></label><label>Abre<input value={window.start} inputMode="numeric" maxLength={5} onChange={event => setHours(previous => previous.map((item, position) => position === index ? { ...item, start: event.target.value } : item))} /></label><label>Fecha<input value={window.end} inputMode="numeric" maxLength={5} onChange={event => setHours(previous => previous.map((item, position) => position === index ? { ...item, end: event.target.value } : item))} /></label><button type="button" className="button secondary" aria-label={`Remover intervalo ${index + 1}`} onClick={() => setHours(previous => previous.filter((_, position) => position !== index))}>Remover</button></div>)}</div>{!hours.length && <p className="admin-empty">Nenhum intervalo: esta unidade ficará sem disponibilidade.</p>}<button type="button" className="button secondary" disabled={hours.length >= 28} onClick={() => setHours(previous => [...previous, { weekday: 1, start: '09:00', end: '18:00' }])}>Adicionar intervalo</button></fieldset><div className="admin-form-actions"><span>Fuso: {schedule.location.timezone}. Alterações que invalidem reservas futuras serão recusadas.</span><button className="button" disabled={busy}>Salvar expediente</button></div></form> : <p className="admin-empty">Cadastre uma unidade para definir horários.</p>}</section>
  </>;
}
