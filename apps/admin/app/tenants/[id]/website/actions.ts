...
export async function websiteAction(id: string, operation: 'drafts' | 'transitions', body: unknown) {
  ...
  try {
    const response = await fetch(new URL(`/v1/admin/tenants/${id}/website/${operation}`, process.env.API_URL || 'http://localhost:4000'), { method: 'POST', cache: 'no-store', redirect: 'manual', signal: AbortSignal.timeout(15000), headers: { 'content-type': 'application/json', cookie: incoming.get('cookie') ?? '', origin: incoming.get('origin') ?? '' }, body: JSON.stringify(body) });
