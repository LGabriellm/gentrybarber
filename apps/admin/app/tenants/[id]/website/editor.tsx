...
export function WebsiteEditor({ id, initial }: { id: string; initial: WebsiteEditorData }) {
  ...
  return <div className="site-studio">
    <div className="studio-toolbar"><div><span className="studio-kicker">SITE WORKSPACE</span><h2>{initial.data.tenant.name}</h2><p>{dirty ? 'Alterações não salvas' : labels[initial.status ?? ''] ?? 'Novo projeto'} · {initial.publishedId ? 'Site publicado' : 'Sem publicação'}</p></div><div className="studio-actions"><button className="button secondary" disabled={busy} onClick={download}>Exportar projeto</button><button className="button secondary" disabled={busy || !initial.themeId || !dirty && !!initial.latestId} onClick={() => void perform('save')}>Salvar rascunho</button><button className="button secondary" disabled={busy || dirty || initial.status !== 'DRAFT'} onClick={() => void perform('approve')}>Aprovar versão</button><button className="button" disabled={busy || dirty || initial.status !== 'APPROVED'} onClick={() => void perform('publish')}>Publicar site</button></div></div>
