...
export default async function Admin() {
  ...
  const metrics = [
    { label: 'Barbearias cadastradas', value: stats.totalTenants, note: `${count(stats.tenants)} com ambiente ativo`, icon: 'shop' as const },
    { label: 'Contas de usuários', value: stats.users, note: `${count(stats.verifiedUsers)} e-mails verificados`, icon: 'users' as const },
    { label: 'Assinaturas ativas', value: stats.subscriptions, note: 'Registros ativos na plataforma', icon: 'subscription' as const },
    { label: 'Ambientes suspensos', value: stats.suspendedTenants, note: 'Acompanhe a situação de acesso', icon: 'paused' as const },
  ];
  const activePercent = stats.totalTenants ? Math.round(stats.tenants / stats.totalTenants * 100) : 0;
  return <AdminLayout activeTab="overview" title="Visão geral" description="Acompanhe sua plataforma e encontre o que precisa para o próximo passo." action={<a className="button" href="/tenants/new"><AdminIcon name="plus" /> Nova barbearia</a>}>
    <div className="admin-metrics">{metrics.map(metric => <article className="admin-metric" key={metric.label}><div className="admin-metric-label"><span>{metric.label}</span><span className="admin-metric-icon"><AdminIcon name={metric.icon} /></span></div><strong>{count(metric.value)}</strong><p>{metric.note}</p></article>)}</div>
    <div className="admin-overview-grid"><section className="admin-panel"><div className="admin-panel-heading"><div><h2>Barbearias recentes</h2><p>Os últimos ambientes cadastrados na plataforma.</p></div><a href="/tenants">Ver todas →</a></div><TenantTable items={tenants.items.slice(0, 5)} compact /></section>
      ...
      <section className="admin-panel" style={{ marginTop: 24 }}><div className="admin-panel-heading"><div><h2>Acesso rápido</h2><p>As próximas ações, em um só lugar.</p></div></div><div className="admin-quick-links"><a href="/users/new"><span>Nova conta<small>Cadastrar um usuário ou administrador</small></span><AdminIcon name="plus" /></a><a href="/plans/new"><span>Novo plano<small>Criar plano comercial</small></span><AdminIcon name="plus" /></a><a href="/users?status=pending"><span>Contas pendentes<small>E-mails não confirmados</small></span><AdminIcon name="arrow" /></a></div></section></div>
    </div><aside className="admin-note"><AdminIcon name="tenant" size={20} /><div><strong>Cada barbearia, um ambiente próprio.</strong><p>O cadastro associa um responsável verificado, um plano e a primeira unidade. A publicação do site é uma etapa separada.</p></div></aside>
  </AdminLayout>;
}
