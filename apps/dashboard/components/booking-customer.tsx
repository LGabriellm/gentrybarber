...
export function CustomerPicker({ slug, selected, onSelect, canCreate, disabled }: { slug: string; selected: CustomerView | null; onSelect: (customer: CustomerView) => void; canCreate: boolean; disabled: boolean }) {
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    const data: CustomerFields = { name: String(form.get('name') || '').trim(), phone: String(form.get('phone') || '').trim(), email: String(form.get('email') || '').trim() || null, notes: String(form.get('notes') || '').trim() || null, whatsappOptIn: form.get('whatsappOptIn') === 'on' };
    if (!data.name || data.name.length > 120 || !/^\+[1-9]\d{7,14}$/.test(data.phone)) { setMessage({ text: 'Informe o nome e o telefone completo no formato internacional, como +5511999999999.', error: true }); return; }
    setBusy(true); setMessage(null);
  ...
  async function changeConsent() {
    ...
    setBusy(true); setMessage(null);
    try {
      const { name, phone, email, notes, version } = selected;
      const customer = await operation<CustomerView>(slug, `customers/${selected.id}`, 'PATCH', { name, phone, email, notes, expectedVersion: version, whatsappOptIn: !selected.whatsappOptInAt });
      onSelect(customer); setResults(null); setMessage({ text: customer.whatsappOptInAt ? 'Autorização para confirmações registrada.' : 'Envios por WhatsApp desativados para este cliente.' });
    } catch { setMessage({ text: 'Não foi possível atualizar a autorização. Busque novamente o cliente antes de tentar.', error: true }); }
    finally { setBusy(false); }
  }
  return <section className="booking-customer" aria-label="Selecionar cliente">
    ...
    {selected && canCreate && <div><p>{selected.whatsappOptInAt ? 'Cliente autorizou confirmações por WhatsApp.' : 'Confirmações por WhatsApp sem autorização registrada.'}</p><button className="button secondary" type="button" disabled={busy || disabled} onClick={() => void changeConsent()}>{selected.whatsappOptInAt ? 'Revogar autorização do WhatsApp' : 'Registrar que o cliente autorizou confirmações por WhatsApp'}</button></div>}
    ...
    {creating && <form className="catalog-form booking-customer-form" onSubmit={create}><fieldset className="catalog-fields" disabled={disabled || busy}><h4>Novo cliente</h4><div className="catalog-form-grid"><label className="catalog-field">Nome do cliente<input name="name" required maxLength={120} autoComplete="name" /></label><label className="catalog-field">Telefone do cliente<input name="phone" type="tel" required maxLength={16} pattern="\+[1-9][0-9]{7,14}" placeholder="+5511999999999" autoComplete="tel" /><small>Use +, código do país, DDD e número, sem espaços.</small></label><label className="catalog-field">E-mail do cliente<input name="email" type="email" maxLength={254} autoComplete="email" /></label></div><label className="catalog-field">Observações do cliente<textarea name="notes" maxLength={2000} rows={2} /></label><label className="catalog-checkbox"><input type="checkbox" name="whatsappOptIn" /> O cliente autorizou receber confirmações de agendamento por WhatsApp neste número.</label><button className="button" type="submit">Salvar cliente</button></fieldset></form>}
  </section>;
}
