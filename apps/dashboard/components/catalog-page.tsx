...
export async function CatalogPage({ slug, resource }: { slug: string; resource: 'services' | 'professionals' }) {
  ...
  const agendaAllowed = entitled && context.permissions.includes('appointments.manage_all') && context.permissions.includes('appointments.read');
  ...
  return <div className="shell catalog-shell">
    ...
    <main className="main">
      ...
      <nav className="tenant-nav" aria-label="Gestão da barbearia">
        {agendaAllowed && <a href={`/tenants/${encodeURIComponent(context.tenant.slug)}/agenda`}>Agenda</a>}
        {agendaAllowed && context.permissions.includes('schedules.manage') && <a href={`/tenants/${encodeURIComponent(context.tenant.slug)}/horarios`}>Horários e bloqueios</a>}
        {entitled && context.permissions.includes('services.manage') && <a href={`/tenants/${encodeURIComponent(context.tenant.slug)}/services`} aria-current={services ? 'page' : undefined}>Serviços</a>}
        {entitled && context.permissions.includes('professionals.manage') && <a href={`/tenants/${encodeURIComponent(context.tenant.slug)}/professionals`} aria-current={!services ? 'page' : undefined}>Profissionais</a>}
      </nav>
