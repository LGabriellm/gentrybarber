...
test('HTML/CSS publicado com preços, agenda real e confirmação repetida sem duplicação', async ({ page }, testInfo) => {
  ...
  await db.professionalService.create({ data: { tenantId: tenant.id, locationId: location.id, professionalId: professional.id, serviceId: service.id } });
  ...
  await expect(site.locator('#agenda')).toBeFocused();
  await expect(site.getByLabel('Unidade', { exact: true })).toHaveCount(0);
  await site.getByLabel('Serviço', { exact: true }).selectOption(service.id);
  ...
  expect(booking.totalCents).toBe(6500);
  ...
  const unavailable = await db.service.create({ data: { tenantId: tenant.id, locationId: location.id, name: 'Sem profissional', durationMinutes: 30, priceCents: 7000 } });
  ...
  await site.getByLabel('Serviço', { exact: true }).selectOption(exclusive.id);
  await expect(site.getByLabel('Profissional', { exact: true })).toHaveCount(0);
  ...
  await site.getByLabel('Serviço', { exact: true }).selectOption(unavailable.id);
  await expect(site.getByText(/Nenhum profissional disponível para este serviço/)).toBeVisible();
  await expect(site.getByLabel('Data', { exact: true })).toBeDisabled();
  expect(errors).toEqual([]);
});
